		...
		for (int j=0;j<global_format.num_of_structs;j++)  {
			if (strcmp(global_format.struct_info[j].identifier, (char*) data+iter) == 0) {
				...
				while (size-iter != 0) {
					...
					uint8_t max_size=0;
					READ_SAFE_FORMAT(max_size, iter, size, data,free(data);,"field")
					uint8_t collabsible_to=0;
					READ_SAFE_FORMAT(collabsible_to, iter, size, data,free(data);,"field")
...
void free_realoc_table(struct redir_table* redir) {
	for (int i=0;i<global_format.num_of_structs;i++) {
...
int32_t generate_format_from_server(int fd, struct format_mask_t* mask, struct redir_table** redir) {
	...
	// init redir table
	*redir = (struct redir_table*) malloc(sizeof(struct redir_table));
	...
	for (int i=0;i<num_of_structs;i++) {
		...
		for (int j=0;j<global_format.num_of_structs;j++) {
			if (strcmp((char*) data+iter, global_format.struct_info[j].identifier) == 0) {
				...
				READ_SAFE_FORMAT(identifier, iter, size, data, free(data);free_realoc_table(*redir);, "struct");

				uint32_t field_count=0;
				while (size-iter > 0) {
					uint8_t field_size;
					READ_SAFE_FORMAT(field_size, iter, size, data,free(data);free_realoc_table(*redir);, "field");
					...
					for (int k=0;k<global_format.struct_info[j].num_of_fields;k++) {
						if (strcmp((char*) data+iter, global_format.struct_info[j].field_info[k].identifier) == 0) {
							iter += field_name_len+1;
							mask->struct_mask[j].field_mask[k] = field_size;
							(*redir)->field_remap[j][k] = field_count;
							field_count++;
							break;
						}
					}
